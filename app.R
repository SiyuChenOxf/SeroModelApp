...
server<-function(input, output,session) {
    ...
    observeEvent( input$goButton2, {
        ...
        med_gamma<-round(quantile(gamma, probs = c(0.025,0.25,0.5,0.75,0.975)),4)
        med_beta<-round(quantile(beta, probs = c(0.025,0.25,0.5,0.75,0.975)),4)
        parameter<-rbind(med_gamma,med_beta)
        parameter<-cbind(c("Infection Fatality Ratio(IFR)","Seroreversion Rate"),parameter)
        parameter<-as.data.frame(parameter)
        
        colnames(parameter)<-c("","2.5% quantitle","25% quantitle","median","75% quantitle","97.5% quantitle")

        sim<-length(beta)
        x<-epsilon<-kft<-matrix(0,sim,n_days)
        posterior <- as.matrix(fit2)
        
        output$table<-renderTable(parameter)
        ...
        data2$lower =ifelse(is.na(data2$lower), data2$value, data2$lower)
        data2$upper =ifelse(is.na(data2$upper), data2$value, data2$upper)
